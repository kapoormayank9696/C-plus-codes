// Diameter Of Node Algorithm Implementation In C++
#include<iostream>
using namespace std;
...
int DiameterOfNodes(Node* root){
    ...
    int leftDiameter = DiameterOfNodes(root->left);
    int rightDiameter = DiameterOfNodes(root->right);

    int diameterThroughRoot = HeightOfNodes(root->left)+HeightOfNodes(root->right);
