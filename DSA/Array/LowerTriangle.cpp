...
// Function to change lower triangle elements into zeroes
void Lower(vector<vector<int>>& arr,int n,int m) {
    for(int i=0;i<n;i++) {
        for(int j=0;j<m;j++) {
            if(i > j) {
                arr[i][j]=0;
...
int main() {
    ...
    display(arr,n,m);
    Lower(arr,n,m);
    cout<<"\nPrint of after lower triangle changes(zeros) of matrix: \n";
    display(arr,n,m);
    return 0;
}
